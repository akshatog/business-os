...
## 2026-09-06 â€” Cross-platform approach
...
**Decision:** React + TypeScript + Vite as the single frontend codebase, wrapped by Electron for desktop and Capacitor for mobile. Not Next.js â€” no server-rendering need for an app that isn't served to web visitors.
**Consequences:** One codebase to maintain across platforms; routing handled by React Router instead of file-based routing.

---

## 2026-09-06 â€” Backend architecture: self-hosted vs. Supabase-native
...
**Consequences:** Slower to first working version than a BaaS approach, but every path touching money/stock data lives in one place that can be read and reasoned about directly â€” matches prior experience and the priority placed on data integrity over setup speed.

---

## 2026-09-06 â€” First vertical
**Context:** Considered starting with general retail/grocery (simpler entities) vs. pharmacy (more complex, but validated interest).
**Decision:** Pharmacy â€” a real medical store expressed interest in the product and offered to refer more clients.
...
## 2026-09-06 â€” Offline support timing
...
## 2026-09-06 â€” Testing approach
**Context:** Considering full test-driven development vs. no formal testing discipline.
**Decision:** Test-required (tests written before/alongside implementation) for business logic â€” sale creation, stock math, permissions/auth. Contract-and-review, not test-first, for UI screens.
...
## 2026-09-06 â€” Money representation
...
**Consequences:** Every money field needs explicit conversion to rupees for display; in exchange, arithmetic across tax/discount/totals can't silently drift due to floating-point rounding â€” directly relevant given this is a finance-handling application.

---

## 2026-09-06 â€” Payment mismatch policy (credit sales)
...
**Decision:** Option B â€” credit sales are supported from Phase 2, not deferred as a later feature. A sale's payment status (`unpaid`/`partially_paid`/`paid`) is derived from its Payment rows rather than stored directly, and additional Payment rows can be recorded against a sale later as repayments.
...
## 2026-09-06 â€” Repayment overpayment
**Context:** Whether a repayment larger than the amount actually owed should be rejected, capped, or stored as forward credit.
**Decision:** A repayment is capped at exactly what's due â€” the system never records or stores an amount beyond the outstanding balance. Any extra cash handed over is change given at the counter, exactly like a normal cash sale, and isn't tracked in the ledger at all.
...
## 2026-09-19 - Deterministic Lock Ordering
**Context:** When processing a sale with multiple items, acquiring locks (`SELECT ... FOR UPDATE`) in arbitrary order can cause database deadlocks if concurrent transactions lock overlapping products in different sequences.
**Decision:** All transactions involving multiple items (e.g., `createSale`) must sort the items by `productId` before iterating and acquiring locks.
**Consequences:** Guarantees that concurrent transactions traverse locks in the same direction, making deadlocks mathematically impossible, at the cost of a tiny sorting overhead.

---

## 2026-09-19 - Transaction Scope for Stock Check & Insert
**Context:** Need to ensure stock doesn't go negative between checking current stock and inserting the movement row.
**Decision:** The sequence LOCK → SUM → VALIDATE → INSERT → COMMIT must all happen within a single database transaction. The lock acquired via `SELECT ... FOR UPDATE` is held until the transaction commits.
**Consequences:** Prevents race conditions completely. `recordStockMovement` must accept an open transaction client (`tx`) from the calling service like `createSale` to participate in the broader transaction.
